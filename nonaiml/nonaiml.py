...
if __name__ == '__main__':
    ...
    print("Initiating No AIML Client...")

    APP = Flask(__name__)

    @APP.route('/.well-known/openchatbot-configuration')
    def index():
        return current_app.send_static_file('.well-known/openchatbot-configuration')

    @APP.route('/api/rest/v2.0/ask', methods=['GET'])
    def receive_message():
        return jsonify(
            {
                "response": {
